# =============================================================================
# Weakly Compressible Maxwell: Straight Channel
# =============================================================================
...
mu_s_cst = ConstantScalar(1.0) # Solvent viscosity
...
beta_cst = ConstantScalar(0.01) # Compressibility factor
...
for step in 1:config.runtime.iterations
    ...
    # For compressible formulation, the deviatoric split implies tau should be dev(tau).
    # To keep the constitutive structure simple initially, we define tau as the extra stress.
    # Note: Bulk viscosity effects are handled by the pressure equation if tau is deviatoric.
    # If tau = 2*mu*D, trace(tau) = 2*mu*div(u).
    # Since div(u) = -beta * p, the trace is non-zero in a simple compressible extension.
    # We maintain tau as defined here for the benchmark.
    ...
    if step == config.runtime.iterations
        report_results("Compressible Maxwell Pressure", res, u, p, txx)
    end
end
