...
config = Configuration(solvers=solvers, schemes=schemes, runtime=Runtime(iterations=1000, write_interval=-1, time_step=1.0), hardware=hardware, boundaries=BCs)

# 2. Solve for Permeability Tensor
dim = 2 # 2D case
K = zeros(dim, dim)
...
for j in 1:dim
    ...
    w_j = solve_stokes_direction(model, config, e_j)
    for i in 1:dim
        K[i, j] = w_j[i]
    end
end
