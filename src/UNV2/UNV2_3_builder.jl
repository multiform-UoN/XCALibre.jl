...
function Gmsh2D_mesh(; scale=1, integer_type=Int64, float_type=Float64)
    ...
    points = Vector{Point{TF}}(undef, n_nodes)
    for i in 1:n_nodes
        tag_to_idx[Int(nodeTags[i])] = TI(i)
        points[i] = Point(SVector{3, TF}(coords[3i-2], coords[3i-1], coords[3i]))
    end
    ...
    # Gmsh 2D element types: 2 (3-node triangle), 3 (4-node quad)
    # UNV loader uses a large Vector indexed by element ID.
    # To keep compatibility with generate(), we find max ID.
    all_element_tags = Int[]
    
    # We also need 1D elements for boundaries
    ...
    boundaryElements = BoundaryLoader{TI}[]
    for (dim, tag) in physGroups
        ...
        loader = BoundaryLoader(name, TI(tag), group_elements)
    ...
    mesh = update_mesh_format(mesh, TI, TF)
    println("Mesh ready!")
    return mesh
end
