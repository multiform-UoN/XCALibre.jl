...
@inline _colval(A::SparseXCSR) = parent(A).colval

@inline get_sparse_fields(A::SparseXCSR) = begin
    Acsr = parent(A)
    Acsr.nzval, Acsr.colval, Acsr.rowptr
end

# Sparse CSR (temporary hack)
@inline _nzval(A::SparseMatricesCSR.SparseMatrixCSR) = A.nzval
@inline _rowptr(A::SparseMatricesCSR.SparseMatrixCSR) = A.rowptr 
@inline _colval(A::SparseMatricesCSR.SparseMatrixCSR) = A.colval
